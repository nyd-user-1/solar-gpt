...
const ANTHROPIC_ICON = (
  <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
    <path d="M17.304 3.541H13.86L7.432 20.46h3.047l1.374-3.768h5.656l1.374 3.768h3.048L17.304 3.54zM12.79 13.695l1.941-5.31 1.94 5.31h-3.88zM6.696 3.541H3.25L9.678 20.46h3.047L6.696 3.54z" />
  </svg>
)

const GEMINI_ICON = (
  <svg className="h-4 w-4" viewBox="0 0 24 24" fill="#4285F4">
    <path d="M12 1.5C12 1.5 8.5 8.5 1.5 12C8.5 15.5 12 22.5 12 22.5C12 22.5 15.5 15.5 22.5 12C15.5 8.5 12 1.5 12 1.5Z" />
  </svg>
)

const GROK_ICON = (
  <svg className="h-4 w-4" viewBox="0 0 2000 1920" fill="currentColor">
  ...
  const inputBox = (
    <div className="relative">
      ...
      <div className="flex flex-wrap items-center gap-1.5 rounded-[28px] border border-[var(--border)] bg-[var(--surface)] pl-2 pr-2 py-2 sm:p-3 shadow-sm transition-shadow hover:shadow-md">
        {/* Plus button — left on desktop (order-2), first on mobile (order-1) */}
        <button
        ...
        {/* Textarea — full-width row on desktop */}
        <textarea
          ref={textareaRef}
          rows={1}
          value={input}
        ...
        {/* Model selector — pushed right on desktop row */}
        <div className="relative order-3 shrink-0 sm:ml-auto" ref={modelRef}>
          ...
          {modelMenuOpen && (
            <div className={`absolute right-0 w-[200px] rounded-xl border border-[var(--border)] bg-[var(--surface)] shadow-xl overflow-hidden py-1 z-20 ${
                ...
                <div className="flex gap-3 overflow-x-auto no-scrollbar -mx-4 px-4">
                  {stateChips.map(s => (
                    <Link
                      key={s.slug}
                      href={`/states/${s.slug}`}
                      className="group shrink-0 w-44 aspect-[16/11] rounded-2xl overflow-hidden relative hover:opacity-90 transition-opacity"
                    >
                      {s.flag_url ? (
                        <img
